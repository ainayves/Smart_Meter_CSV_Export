...
def _purge_modules():
    """Remove all 'app.*' modules from the cache to ensure a clean import."""
...
def test_validate_dates_only_ok_and_errors(json_file, monkeypatch):
    # Prepare env before import
    monkeypatch.setenv("DATA_SOURCE", "json")
    monkeypatch.setenv("JSON_FILE", str(json_file))

    _purge_modules()  # full purge

    import app.settings as settings

    # settings reads env; no reload needed for a fresh import
    import app.utils as utils

    # OK: start in the past, end after start, range >= 1 minute and <= 1 year
...
def test_json_provider_filters_and_sort(json_file, monkeypatch):
    ...
    # Window [00:00, 00:02) -> 2 rows for SM-001 (00:00, 00:01)
    start = datetime(2024, 1, 1, 0, 0, 0, tzinfo=timezone.utc)
    end = datetime(2024, 1, 1, 0, 2, 0, tzinfo=timezone.utc)
    rows = list(prov.iter_readings("SM-001", start, end))
    assert len(rows) == 2
    assert rows[0][0] < rows[1][0]  # sorted by time

    # Window with no rows -> 0
    rows2 = list(
        prov.iter_readings(
